        ...
        if($scope.validate_fees_payment()) {
            ...
            $http({
                ...
                if (data.result == 'error'){
                    ...
                    document.location.href = '/fees/fee_receipt/?student='+$scope.payment_installment.student_id+'&head='+$scope.payment_installment.head_id+'&amount='+$scope.payment_installment.paid_amount+'&fine='+$scope.payment_installment.fine;
                }
                                    ...
                                    if($scope.fees_structure.fees_head[i].installments[j].type == "Standard Payment" && $scope.fees_structure.fees_head[i].installments[k].type  == "Late Payment"){
                                        if(end_date >= start_date_current){
                                            ...
                                            if ((start_date - end_date)/(1000*60*60*24) > 1) {
                                                $scope.validation_error = "Please check the start date of the payment type "+$scope.fees_structure.fees_head[i].installments[j].type+" and end date for the payment type "+$scope.fees_structure.fees_head[i].installments[j-1].type+' in '+$scope.fees_structure.fees_head[i].head;
                                                return false;
                                            }
                                        }
                                    }
